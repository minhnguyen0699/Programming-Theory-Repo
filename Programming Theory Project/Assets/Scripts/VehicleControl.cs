...
public class VehicleControl : MonoBehaviour
{
    ...
    void OnCollisionEnter(Collision collision)
    {
        ...
        {
            updateHealth(-1);
        } else if (collision.gameObject.CompareTag("Enemy")) { updateHealth(-5); }
    ...
    {
        healthControl += healthPoint;
        if (healthControl < 0)
        {
            healthControl = 0;
        } else if (healthControl >100) {
            healthControl = 100;
        }
    }
    private void OnTriggerEnter(Collider other)
    {
        if (other.gameObject.CompareTag("heart")&& healthControl<100)
        {
            updateHealth(10);
